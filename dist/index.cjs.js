...
var Link$1 = React3__namespace.forwardRef(
...
const ScrollToTopButton = () => {
  const [isVisible, setVisible] = React3.useState(false);
  React3.useEffect(() => {
...
const Image = ({
  ...
  const [insertImagePlaceholder, setInsertImagePlaceholder] = React3.useState(false);
  React3.useEffect(() => {
    if (src)
...
const Label = ({ htmlFor, children, required = false, className = "" }) => {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("label", { htmlFor, className: clsx("flex flex-col gap-1", className), children: React3.Children.map(
    children,
...
const InputText = ({
  ...
  var _a;
  const [internalValue, setInternalValue] = React3.useState((_a = value != null ? value : defaultValue) != null ? _a : "");
...
const Login = () => {
  const [showLocalAccountLoginForm, setShowLocalAccountLoginForm] = React3.useState(false);
...
const Azure = () => {
  const [formStep, setFormStep] = React3.useState(1);
...
const Snackbar = ({
  ...
  const [isDisplayed, setIsDisplayed] = React3.useState(true);
  const [progress, setProgress] = React3.useState(100);
